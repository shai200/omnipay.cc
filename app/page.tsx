...
const faqItems = [
  ...
  },
  {
    ...
    a: "Copy Form↔Slot pair puts a Form FP ↔ Slot FP citation (with = or ≠) on the clipboard so you can cite the live form vs Save reminder — defaults untouched. Export form downloads the live form as omnipay-reminder-form.json (distinct from Export defaults / Export reminder) — slot untouched. Paste form / Import form reload that JSON into the live form only; Paste/Import defaults also accept it; Import reminder can restore it into the slot. Fingerprints never leave your browser.",
  },
  {
    q: "What is Paste form / Import form / Swap form ↔ defaults?",
    a: "Paste form reads Export form JSON (or Export defaults / #omn-reminder=) from the clipboard/field and applies it to the live form only — slot untouched. Import form loads the same .json via file picker into the form only. Swap form ↔ defaults exchanges the live form with Clear form defaults using a session buffer (click again to restore the previous form) — Save reminder slot stays untouched. Distinct from Swap reminder ↔ form, which exchanges with the Save reminder slot. Fingerprints never leave your browser.",
  },
  {
